...
const next = await request(`/v1/diagnostic/next?learnerId=${encodeURIComponent(learner.id)}`);
...
const dashboard1 = await request(`/v1/dashboard?learnerId=${encodeURIComponent(learner.id)}`);
...
const readiness = await request(`/v1/readiness?learnerId=${encodeURIComponent(learner.id)}`);
assert.equal(typeof readiness.scorePercent, 'number');
assert.equal(typeof readiness.coverageConfidence, 'number');

const studyPlan = await request(`/v1/study-plan/today?learnerId=${encodeURIComponent(learner.id)}`);
...
const snapshot = await request(`/v1/learners/${encodeURIComponent(learner.id)}/snapshot`);
...
const dashboard2 = await request(`/v1/dashboard?learnerId=${encodeURIComponent(learner.id)}`);
