...
export function ProgressRing({ score, label, size = 172, stroke = 10, color = theme.color.primary }: { score: number; label?: string; size?: number; stroke?: number; color?: string }) {
  ...
  return <View style={{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }}>
    ...
    <View style={styles.ringCenter}><View style={styles.ringValueRow}><Text style={[styles.ringValue, size < 150 && { fontSize: 42, lineHeight: 47 }]}>{score}</Text><Text style={[styles.ringPercent, size < 150 && { fontSize: 20 }]}>%</Text></View>{label ? <Text numberOfLines={1} style={[styles.ringLabel, label.length > 10 && { fontSize: 13 }, { color }]}>{label}</Text> : null}</View>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  ringCenter: { alignItems: 'center', justifyContent: 'center' }, ringValueRow: { flexDirection: 'row', alignItems: 'flex-end' }, ringValue: { color: theme.color.text, fontSize: 62, lineHeight: 66, fontWeight: '400', fontFamily: 'Georgia', letterSpacing: -2 }, ringPercent: { color: theme.color.text, fontSize: 27, lineHeight: 38, marginBottom: 5, fontWeight: '500', fontFamily: 'Georgia' }, ringLabel: { fontSize: 17, fontWeight: '600', marginTop: -2 },
  ...
  quickAction: { flex: 1, minHeight: 78, backgroundColor: theme.color.surface, borderWidth: 1, borderColor: theme.color.border, borderRadius: theme.radius.md, alignItems: 'center', justifyContent: 'center', gap: 8, padding: 10, ...theme.shadow.soft }, quickActionText: { color: theme.color.text, fontSize: 13, lineHeight: 17, fontWeight: '600', textAlign: 'center' },
  trustCard: { minHeight: 64, flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: theme.color.primaryPale, borderWidth: 1, borderColor: theme.color.border, borderRadius: theme.radius.md, paddingHorizontal: 16, paddingVertical: 12 }, trustTitle: { color: theme.color.text, fontSize: 15, fontWeight: '700' }, trustSubtitle: { color: theme.color.textMuted, fontSize: 13, lineHeight: 18, marginTop: 3 },
