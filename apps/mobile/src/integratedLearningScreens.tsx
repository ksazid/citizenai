...
function LearnConcept({ navigate, goBack }: Props) {
  ...
  return <View style={s.screen}><AppHeader onBack={goBack} days={`${rt.daysUntilExam} days until test`} /><View style={s.progress}><Text style={s.progressText}>Today’s plan · Learn</Text><View style={{ flex: 1 }}><ProgressBar value={1} max={4} /></View></View><Text style={typography.h1}>{concept?.title ?? 'Learn concept'}</Text><Text style={s.subtitle}>Focus on the underlying idea, not a memorised sentence.</Text><Card><View style={s.lessonRow}><IconTile name="people-outline" /><View style={{ flex: 1 }}><Text style={s.lessonTitle}>Core idea</Text><Text style={s.lessonText}>{concept?.id === 'elections' ? 'Voters elect Members of Parliament to represent constituencies.' : 'Build a clear mental model of this concept before testing it.'}</Text></View></View><View style={s.lessonRow}><IconTile name="git-compare-outline" tone="teal" /><View style={{ flex: 1 }}><Text style={s.lessonTitle}>Distinction</Text><Text style={s.lessonText}>{concept?.id === 'elections' ? 'Voters do not directly elect the Prime Minister.' : 'Separate this concept from nearby ideas that are easy to confuse.'}</Text></View></View><View style={s.lessonRow}><IconTile name="sparkles-outline" /><View style={{ flex: 1 }}><Text style={s.lessonTitle}>Next evidence</Text><Text style={s.lessonText}>CitizenAI will test this with unseen wording so one remembered question cannot inflate mastery.</Text></View></View></Card><Button label="Check understanding" onPress={() => navigate('question')} /><BottomTabs active="learn" navigate={navigate} /></View>;
}

function CompareConcepts({ navigate, goBack }: Props) {
  const rt = useCitizenAI();
  return <View style={s.screen}><AppHeader onBack={goBack} days={`${rt.daysUntilExam} days until test`} /><View style={s.progress}><Text style={s.progressText}>Today’s plan · Compare</Text><View style={{ flex: 1 }}><ProgressBar value={2} max={4} /></View></View><Text style={s.compareTitle}>Parliament vs Government</Text><Text style={s.subtitle}>The remediation engine selected a contrast because this is a concept-confusion pattern.</Text><Card style={s.matrix}><View style={s.headers}><View style={s.header}><IconTile name="business-outline" /><Text style={[s.headerText, { color: theme.color.primary }]}>Parliament</Text></View><Text style={s.vs}>VS</Text><View style={s.header}><IconTile name="business-outline" tone="teal" /><Text style={[s.headerText, { color: theme.color.tealDark }]}>Government</Text></View></View><View style={s.columns}><View style={s.column}><Cell icon="create-outline" text="Makes and scrutinises laws" /><Cell icon="people-outline" text="Includes MPs and House of Lords" /><Cell icon="scale-outline" text="Holds Government accountable" /></View><View style={s.vertical} /><View style={s.column}><Cell tone="teal" icon="settings-outline" text="Runs the country" /><Cell tone="teal" icon="person-outline" text="Led by the Prime Minister" /><Cell tone="teal" icon="checkmark-circle-outline" text="Implements policy" /></View></View></Card><Card tone="soft" style={s.callout}><IconTile name="sparkles-outline" /><View style={{ flex: 1 }}><Text style={s.lessonTitle}>Why CitizenAI sent you here</Text><Text style={s.lessonText}>Wrong answers that reverse these roles trigger compare → recall → unseen variant rather than another random question.</Text></View></Card><Button label="Check understanding" onPress={() => navigate('recall')} /><Pressable style={s.save}><AppIcon name="bookmark-outline" size={21} color={theme.color.text} /><Text style={s.saveText}>Save for later</Text></Pressable><BottomTabs active="learn" navigate={navigate} /></View>;
}
...
const s = StyleSheet.create({
  screen: { flex: 1, gap: 12 }, progress: { flexDirection: 'row', alignItems: 'center', gap: 18 }, progressText: { color: theme.color.text, fontSize: 14, fontWeight: '600' }, subtitle: { color: theme.color.textMuted, fontSize: 15, lineHeight: 22 },
  lessonRow: { flexDirection: 'row', gap: 12, alignItems: 'flex-start', paddingVertical: 11 }, lessonTitle: { color: theme.color.text, fontSize: 16, fontWeight: '700' }, lessonText: { color: theme.color.textMuted, fontSize: 13, lineHeight: 19, marginTop: 4 },
  compareTitle: { color: theme.color.text, fontSize: 30, lineHeight: 36, fontWeight: '700' }, matrix: { padding: 14 }, headers: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: theme.color.border }, header: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8 }, headerText: { fontSize: 17, fontWeight: '700' }, vs: { color: theme.color.textMuted, fontSize: 12, fontWeight: '700', marginHorizontal: 6 }, columns: { flexDirection: 'row' }, column: { flex: 1, paddingHorizontal: 4 }, vertical: { width: 1, backgroundColor: theme.color.border }, cell: { minHeight: 87, gap: 7, justifyContent: 'center', borderBottomWidth: 1, borderBottomColor: theme.color.border }, cellText: { color: theme.color.text, fontSize: 13, lineHeight: 18 }, callout: { flexDirection: 'row', gap: 12, alignItems: 'center' }, save: { minHeight: 52, borderWidth: 1, borderColor: theme.color.border, borderRadius: 16, flexDirection: 'row', gap: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: theme.color.surface }, saveText: { color: theme.color.text, fontSize: 15, fontWeight: '600' },
  recall: { minHeight: 220, alignItems: 'center', justifyContent: 'center', gap: 14 }, recallPrompt: { color: theme.color.textMuted, fontSize: 18, lineHeight: 27, textAlign: 'center' }, recallAnswer: { color: theme.color.text, fontSize: 21, lineHeight: 28, fontWeight: '700', textAlign: 'center' }
});
