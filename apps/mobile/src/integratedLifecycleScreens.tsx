...
function MockQuestion({ navigate, goBack }: Props) {
  ...
  return <View style={s.screen}><AppHeader onBack={goBack} /><View style={s.rowBetween}><Text style={s.label}>Question {index + 1} of 24</Text><Text style={s.meta}>45 min max</Text></View><ProgressBar value={index + 1} max={24} /><Text style={[typography.h1, { marginTop: 12 }]}>{q.stem}</Text><View style={s.options}>{q.options.map((o: any) => <Pressable key={o.id} onPress={() => setSelected(o.id)} style={[s.option, selected === o.id && s.optionSelected]}><View style={[s.radio, selected === o.id && s.radioSelected]} /><Text style={s.optionText}>{o.text}</Text></Pressable>)}</View><Button label={isLast ? 'Review answers' : 'Next'} disabled={!selected} onPress={() => { if (!selected) return; rt.answerMock(selected); setSelected(null); if (isLast) navigate('mock-review'); }} /></View>;
}
...
function SourceInfo({ navigate, goBack }: Props) {
  return <View style={s.screen}><AppHeader onBack={goBack} /><Text style={typography.h1}>Sources & version</Text><Card tone="soft"><Text style={s.cardTitle}>UK Knowledge Pack</Text><Text style={s.copy}>The integration currently uses a non-production seed pack to exercise the certified engine. Production questions remain blocked unless fact provenance is approved.</Text></Card><Card><ListRow title="Canonical facts" meta="Must be approved with evidence" icon="shield-checkmark-outline" /><ListRow title="Question answers" meta="Must resolve to publishable facts" icon="link-outline" /><ListRow title="AI role" meta="May propose wording, never establish facts" icon="sparkles-outline" hideDivider /></Card><Button secondary label="Back to profile" onPress={() => navigate('profile')} /></View>;
}
...
const s = StyleSheet.create({
  ...
  label: { color: theme.color.text, fontSize: 15, fontWeight: '600' }, meta: { color: theme.color.textMuted, fontSize: 12, lineHeight: 17 },
  copy: { color: theme.color.textMuted, fontSize: 14, lineHeight: 21, marginTop: 7 },
  heroMetric: { color: theme.color.text, fontSize: 48, lineHeight: 52, fontWeight: '700' }, heroTitle: { color: theme.color.text, fontSize: 20, fontWeight: '700', marginTop: 4 },
  ...
  metrics: { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'stretch', gap: 4 },
  score: { color: theme.color.text, fontSize: 48, lineHeight: 54, fontWeight: '700', marginTop: 14 },
  cardTitle: { color: theme.color.text, fontSize: 18, fontWeight: '700' }, readinessRow: { flexDirection: 'row', alignItems: 'center', gap: 18, marginTop: 8 }, confidence: { color: theme.color.tealDark, fontSize: 13, fontWeight: '700', marginTop: 10 },
  passHero: { flexDirection: 'row', alignItems: 'center', gap: 15, padding: 17 }, passCopy: { flex: 1 }, passTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8 }, passTitle: { color: theme.color.text, fontSize: 25, fontWeight: '700' },
  untilCard: { minHeight: 80, borderWidth: 1, borderColor: theme.color.border, borderRadius: 18, backgroundColor: theme.color.surface, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 12 }, untilTitle: { color: theme.color.text, fontSize: 16, fontWeight: '700' },
  outcomeRow: { flexDirection: 'row', gap: 12, marginTop: 14 }, outcome: { flex: 1, minHeight: 150, borderWidth: 1, borderColor: theme.color.border, borderRadius: 20, backgroundColor: theme.color.surface, alignItems: 'center', justifyContent: 'center', gap: 12 }, outcomeTitle: { color: theme.color.text, fontSize: 18, fontWeight: '700' },
  center: { alignItems: 'center', gap: 12, paddingVertical: 28 }, consent: { flexDirection: 'row', gap: 12, alignItems: 'flex-start', padding: 16, borderWidth: 1, borderColor: theme.color.border, borderRadius: 18, backgroundColor: theme.color.surface }, checkbox: { width: 24, height: 24, borderRadius: 7, borderWidth: 1.5, borderColor: theme.color.border, alignItems: 'center', justifyContent: 'center' }, checkboxOn: { backgroundColor: theme.color.primary, borderColor: theme.color.primary }, consentTitle: { color: theme.color.text, fontSize: 14, fontWeight: '700' }
});
