...
export const UK_PACK_V3_MANIFEST = Object.freeze({
  id:'GB-2026.09.02-candidate.3', countryCode:'GB', version:'2026.09.02-candidate.3', status:'review',
  independentlyAuthored:true, basePackId:UK_PACK_V2.manifest.id,
  ...
  coverage:Object.freeze({
    status:'coverage_candidate', officialGuideAligned:false, examComplete:false, activationAllowed:false,
    publicSourceConceptCount:59, questionCount:177,
...
const extensionSources=sourceRows.map(([id,url,title,sourceType,dynamic,contentHash])=>Object.freeze({...createSource({id,packId:UK_PACK_V3_MANIFEST.id,url,sourceType,title,retrievedAt:RETRIEVED_AT,contentHash,dynamic,verificationStatus:'approved'}),hashBasis:'legacy_url_title_fingerprint_pending_body_snapshot_backfill'}));
...
const extensionConcepts=Object.freeze(rows.map(([id,domainId,title,importance,baseDifficulty])=>Object.freeze({...createConcept({id,domainId,key:id,title,importance,baseDifficulty,status:'approved'}),studyMinutes:3})));
...
const falseOptions=[
  'This statement belongs to a different topic and is not supported by this concept’s source.',
  'This claim is not supported by the authoritative source linked to this concept.',
  'This describes a direct election of the UK Prime Minister, which is unrelated to this concept.'
];
function makeVariants(concept,fact){
  const stems=[`Which statement best describes ${concept.title}?`,`Which verified summary is associated with ${concept.title}?`,`Choose the correct description of ${concept.title}.`];
  const positions=[0,1,2];
  return stems.map((stem,index)=>{
    const options=falseOptions.map((text,i)=>({id:['a','b','c','d'][i],text}));
    options.splice(positions[index],0,{id:['a','b','c'][index],text:fact.canonicalValue});
    return Object.freeze({id:`${concept.id}-verified-q${index+1}`,conceptId:concept.id,factId:fact.id,packVersion:UK_PACK_V3_MANIFEST.version,questionType:'multiple_choice',stem,options:Object.freeze(options),correctOptionId:['a','b','c'][index],difficulty:Math.min(1,concept.baseDifficulty+index*0.04),variantId:`${concept.id}-verified-v${index+1}`,explanation:fact.canonicalValue,status:'approved',provenanceStatus:'verified'});
  });
}
const extensionQuestions=Object.freeze(extensionConcepts.flatMap((concept)=>makeVariants(concept,extensionFacts.find((fact)=>fact.conceptId===concept.id))));

const remapBaseSources=UK_PACK_V2.sources.map((source)=>Object.freeze({...source,packId:UK_PACK_V3_MANIFEST.id}));
export const UK_PACK_V3=Object.freeze({manifest:UK_PACK_V3_MANIFEST,sources:Object.freeze([...remapBaseSources,...extensionSources]),evidence:Object.freeze([...UK_PACK_V2.evidence,...extensionEvidence]),concepts:Object.freeze([...UK_PACK_V2.concepts,...extensionConcepts]),facts:Object.freeze([...UK_PACK_V2.facts,...extensionFacts]),questions:Object.freeze([...UK_PACK_V2.questions,...extensionQuestions])});

export function validateUkPackV3(){
  const errors=[]; const base=validateUkPackV2(); if(!base.ok) errors.push(...base.errors.map((e)=>`base:${e}`));
  ...
  const evidenceById=new Map(UK_PACK_V3.evidence.map((e)=>[e.id,e])); const sourcesById=new Map(UK_PACK_V3.sources.map((s)=>[s.id,s]));
  ...
  const stems=new Set();
  for(const question of UK_PACK_V3.questions){const validation=validateQuestionProvenance({question,factsById});if(!validation.ok)errors.push(`question_provenance:${question.id}:${validation.reason}`);if(question.options.length!==4)errors.push(`question_options:${question.id}`);if(!question.options.some((o)=>o.id===question.correctOptionId))errors.push(`question_correct_option:${question.id}`);if(stems.has(question.stem))errors.push(`duplicate_question_stem:${question.id}`);stems.add(question.stem);const correctText=question.options.find((o)=>o.id===question.correctOptionId)?.text;if(question.options.filter((o)=>o.text===correctText).length!==1)errors.push(`ambiguous_duplicate_answer:${question.id}`);}
  if(UK_PACK_V3.concepts.length!==59)errors.push(`concept_count:${UK_PACK_V3.concepts.length}`); if(UK_PACK_V3.questions.length!==177)errors.push(`question_count:${UK_PACK_V3.questions.length}`); if(UK_PACK_V3.manifest.coverage.examComplete||UK_PACK_V3.manifest.coverage.activationAllowed)errors.push('coverage_gate_must_remain_closed');
